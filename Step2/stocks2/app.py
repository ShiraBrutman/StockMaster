...
def add_stock():
    ...
    result = stocks2_collection.insert_one(stock)
...
def get_stocks():
    try:
        all_stocks = list(stocks2_collection.find({}))
...
@app.route('/stocks2/<stock_id>', methods=['GET'])
def get_stock_by_id(stock_id):
    try:
        stock = stocks2_collection.find_one({'_id': ObjectId(stock_id)})
        if not stock:
...
@app.route('/stocks2/<stock_id>', methods=['PUT'])
def update_stock(stock_id):
    try:
        ...
        result = stocks2_collection.update_one(
...
@app.route('/stocks2/<stock_id>', methods=['DELETE'])
def delete_stock(stock_id):
    try:
        # Delete the stock by its `_id`
        result = stocks2_collection.delete_one({'_id': ObjectId(stock_id)})
...
@app.route('/stocks2/stock-value/<stock_id>', methods=['GET'])
...
@app.route('/stocks2/portfolio-value', methods=['GET'])
def get_portfolio_value():
    try:
        total_value = 0.0
        stocks = list(stocks2_collection.find({}, {'_id': 0}))
